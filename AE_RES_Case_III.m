...
if 0 | ~exist('rep') | rep ~=1;
    figure
    plot(u_w(200:1/dt:end,end),u_w(200:1/dt:end,end).*Ct(200:1/dt:end,end)/max(max(u_w))^3,'k.','Color',[0.4 0.4 0.4])
    hold all
    
    % fit linear curve
    temp = corrcoef(u_w(200:end,101),Ct(200:end,101).*u_w(200:end,101)/max(max(u_w))^3);
    R_lin = temp(2);
    lin_coef = polyfit(u_w(200:end,101),Ct(200:end,101).*u_w(200:end,101)/max(max(u_w))^3,1);
    plot([0:0.01:10],polyval(lin_coef,[0:0.01:10]),'k','linewidth',2)
    
    if 0
        % fit cubic curve
        fitt = polyfit(u_w(200:end,101).^3,Ct(200:end,101).*u_w(200:end,101)/max(max(u_w))^3,1);
        plot([0:1:1000].^(1/3),(polyval(fitt,[0:1:1000])),'g','linewidth',2)
        temp=corrcoef(u_w(200:end,101).^3,Ct(200:end,101).*u_w(200:end,101)/max(max(u_w))^3);
        cub_fit = temp(2)^2;
    end
    
    hold all
    % plot(1:10,[1:10].^3/10^3,'k-.','linewidth',2)
    
    xlabel('Wind speed [m/s]')
    ylabel('Q/max(Q_u) at x=101m [-]')
    % ylim([0 1.1])
    xlim([0 10])
    set(gcf,'PaperPosition',[0.634517 6.34517 7 5])
    
    Total_source = sum(source(1,:))
    Average_concentration = lin_coef
end

%% FITTING !!
% Lets plot q and v averaged over 1 second.
% We have Uwind and Ct, CT is somehow ionfluenced by dt ???
%  plot(u_w(200:1/dt:end,end),u_w(200:1/dt:end,end).*Ct(200:1/dt:end,end)/max(max(u_w))^3,'k.','Color',[0.4 0.4 0.4])

%% laten we eerst een beetje middelen en dan relaties fitten
...
if 0
    figure
    [V_th3,c_3,P_3] = powerfit((u_w(1:1/dt:end,end))',(VS*u_w(1:1/dt:end,end).*Ct(1:1/dt:end,end))',1,'3rd')
    plot(1:15,((1:15)-U_th).^3)
    
    
    [V_th1,c_1,P_1] = powerfit((u_w(1:1/dt:end,end))',(VS*u_w(1:1/dt:end,end).*Ct(1:1/dt:end,end))',1,'lin')
end
%%
if 1
...
%     p(2,2).select()
%     plot(Ct(1:1/dt:end,101),'linewidth',2,'Color',[0.4 0.4 0.4])
%     xlim([0 3650])
%     set(gca, 'xtick', [])
%     box on
%     vline(times,'k:')
